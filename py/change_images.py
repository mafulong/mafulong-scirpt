...
user_name = "mafulong";
## 仓库名
github_repository = "mdPic";

## git仓库在本机的位置
git_repository_folder = "/Users/mafulong/github/mdPic"

## 存放图片的git文件夹路径
git_images_folder = "/Users/mafulong/github/mdPic/images"
...
def get_http_image(image_url):
    ...
    image_data = requests.get(image_url, headers=headers, proxies={"http":"http://127.0.0.1:7890", "https:":"=http://127.0.0.1:7890"}).content
    if not image_data:
        return image_info
    # 创建临时文件
    ...
    new_image_url = "https://raw.githubusercontent.com/" + user_name + "/" + github_repository + "/master/" + \
                    git_images_folder.split("/")[-1] + "/" + new_image_path_and_name.split("/")[-1]
    image_info = {
        "image_url": image_url,
    ...
    with open(md_file, "r+") as f:
        ...
        for image_url in image_urls:
            ...
            # 处理网络图片
                ...
                # 不爬取某些内容
                ignore_start_urls = ["https://raw.githubusercontent.com/mafulong/mdPic",
                                     "https://cdn.jsdelivr.net/gh/mafulong"]
                ignore = False
                for ignore_start_url in ignore_start_urls:
                    if image_url.startswith(ignore_start_url):
                        ignore = True
                        break
                if not ignore:
                    try:
...
def handle(path):
    ...
    for md_file in md_files:
        ...
        # print(md_file)
        # print("hhhh")
        go(get_images_from_md_file, (md_file,))

    # git_push_to_origin()

def go(target=None, param=()):
    print(param)
    try:
        t = threading.Thread(target=target, args=param)
        t.setDaemon(True)
        t.start()
...
def main():
    handle("/Users/mafulong/mafulong.github.io/_posts/Tech/")

if __name__ == "__main__":
    main()
    import time
    time.sleep(1000)
    # go(get_images_from_md_file, ("/Users/mafulong/mafulong.github.io/_posts/Tech/Tools/2020-11-29-vpn.md",))
